...
- FAQ (or other) schema data that is a verbatim duplicate of the corresponding body copy (Redundancy check) — rewrite the body copy to read conversationally, distinct from the schema field, so the two aren't the same sentence twice. Schema stays as the structured data point, body copy becomes natural-language prose.
- A customer quote (testimonial, "in their words," a quoted objection) that reads smoothed into corporate voice rather than the customer's actual phrasing from Loren's research — restore the verbatim customer language.
- Generic placeholder alt-text or captions ("representative image," "stock photo," "placeholder") — rewrite to describe the specific, proprietary asset the section actually calls for.
...
Loop this yourself: fix, re-scan the file, fix again if needed, capped at 2 internal passes, same cap discipline as every other stage in this pipeline. This all happens inside your one dispatch. Overwrite the draft file in place when you're done.

State plainly in your output everything you fixed (so the orchestrator can see what changed without re-reading the whole diff), including anything you resolved with a best-effort judgment call rather than a clean fix. There is no separate findings list for the orchestrator to act on. Your output is either PASS with an Auto-fixed list, or, in the rare case you hit your 2-pass cap with something still imperfect, PASS with an Auto-fixed list plus a one-line note on what's still soft/imperfect and why you shipped it anyway. You never return FAIL and you never return an open finding.
...
- Redundancy check: FAQ (or other) schema data is not a verbatim duplicate of the body copy it corresponds to (soft, force a rewrite of the body copy to be more conversational when found)
