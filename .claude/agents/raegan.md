...
- **Schema Separation:** schema fields must be distinct from body copy, not a copy-paste of it. Use schema fields for technical/structured data points (hours, price, raw Q&A pairs, NAP data) written as data, and use body copy for conversational, natural-language prose. An FAQ answer in the body should read like a conversational answer; the same fact in FAQPage schema should be stated as the structured data point itself, not a duplicate of the exact same sentence.
...
- **Schema Separation check:** for every schema field you filled (especially FAQPage Q&A pairs), confirm it is not an exact duplicate of the corresponding body-copy sentence. Where you find a duplicate, rewrite the body-copy version to read conversationally and leave the schema field as the structured data point.
